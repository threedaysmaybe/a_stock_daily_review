...
for name in selected_hot[:5] if selected_hot else available_hot[:5]:
    ...
    with st.expander(f"🕵️ {name} — 近{selected_days}日操作（{len(name_df)}笔）", expanded=(name == (selected_hot[0] if selected_hot else available_hot[0]))):
